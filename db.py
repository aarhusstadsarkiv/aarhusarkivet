...
def update_user_role(user_id, new_role):
    ...
    key = {"user_id": user_id}
    updates = {"attribute": "roles", "value": new_roles}
...
def update_search(user_id, created, description):
    ...
    if resp:
        ...
        return {"error": True, "msg": "Unable to update search. Try again later."}
...
def get_order(user_id, resource_id):
    return _get_item("orders", {"user_id": user_id}, {"resource_id": resource_id})
...
def _delete_item(table_name, key, return_item=False):
    ...
    response = table.delete_item(
        Key=key, ReturnValues="ALL_OLD" if return_item else "NONE"
    )
